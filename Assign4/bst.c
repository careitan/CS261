...
{
	if (cur == NULL){
		struct Node *addThis = malloc(sizeof(struct Node));
		addThis->val = val;
		addThis->right = NULL;
		addThis->left = NULL;
	...
	{
		int CompTest = compare(cur->val, val);
		switch (CompTest)
		{
			// Current node is Greater than the new value.
		case 1:
			cur->left = _addNode(cur->left, val);
			break;
			// Current node is Less than the new value.
		case -1:
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	if (tree != NULL)
	{
		...
		while (Current != NULL)
		{
			int TestComp = compare(Current->val, val);
