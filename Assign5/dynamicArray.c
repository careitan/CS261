 ...
 * Date: 07/30/2015
 * Solution description: Heap and dynamic array
 */
...
void addHeap(DynArr *heap, TYPE val)
{
    ...
    {
        parent = (pos-1)/2;
        if (compare(heap->data[pos],heap->data[parent]) == -1)
        {
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   /* FIXME */
    ...
    {
        if (compare(heap->data[pos],heap->data[left]) == 1)
        {
...
void removeMinHeap(DynArr *heap)
{
   /* FIXME */
    ...
    putDynArr(heap,0,heap->data[last]);
...
void sortHeap(DynArr *heap)
{
   /* FIXME */
    ...
    {
        swapDynArr(heap,index,0);
        _adjustHeap(heap,index,0);
    }
    //below used if code should be ordered from lowest to highest
/*    for (int index = 0; index < heap->size/2; index++)
    {
        swapDynArr(heap,index,heap->size-index-1);
    }*/
}
