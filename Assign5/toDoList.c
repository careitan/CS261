 ...
 * Date: 07/30/2015
 * Solution description: Functions specific to toDoList
 */
...
void loadList(DynArr *heap, FILE *filePtr)
{
	...
	char desc[TASK_DESC_SIZE]/*, *nlptr*/; //commented out to compile without warnings
	int priority;
...
void printList(DynArr *heap)
{
    ...
    for (int index = 0; index < sizeDynArr(heap); index++)
    {
        addDynArr(temp,getDynArr(heap,index));
    }
    sortHeap(temp);

    for (int index = 0; index < sizeDynArr(temp); index++)
    {
        print_type(getDynArr(temp,index));
    }
...
{
    /*FIXME: write this*/
    struct Task* leftValue = (struct Task*) left;
    struct Task* rightValue = (struct Task*) right;

    if (leftValue->priority < rightValue->priority)
    {
        return -1;
    }
    else if (leftValue->priority > rightValue->priority)
    {
        return 1;
    }
    else if (leftValue->priority == rightValue->priority)
