...
int main (int argc, const char * argv[]) {
	...
	// REF: Piazza Discussion @308 from CS261_400 Summer 2015
	// Open file
	...
	if (fileptr)
	{
		...
		while (word != NULL)
		{
			if (word)
			{
				...
				{
					int newValue = atMap(hashTable, word);
					newValue++;
					insertMap(hashTable, word, newValue);
				}
				// Otherwise, insert the word with starting value of 1
				else
				{
					insertMap(hashTable, word, NULL);
				}
			}
			word = getWord(fileptr);
		}
	...
	// Generate the Concordance output text per assignment requirements.
	for (int i = 0; i < hashTable->tableSize; i++)
	{
		struct hashLink *tempLink = (hashLink *)hashTable->table[i];
