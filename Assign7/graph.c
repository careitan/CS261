...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	int i;
	/* Define the Temp Stack-Queue for the Function. 
	 * Going to use the Front as the TOP of the Stack */
	struct cirListDeque *stack = malloc(sizeof(struct cirListDeque));
	initCirListDeque(stack);

	/* Establish Temp Vertex to use in function to slide through stack */
	Vertex* CurrentVertex = source;

	/* Set the initial visited flag*/
	CurrentVertex->isVisited = 1;

	/* Add the source Vertex to the Stack for examination. */
	addFrontCirListDeque(stack, CurrentVertex);

	/* Loop through the neighbors to check for destination */
	while (isEmptyCirListDeque(stack)==0)
	{
		for (i = 0; i < CurrentVertex->numNeighbors; i++)
		{
			/* if a neighbor is the destination, return 1 as a path exists */
			if (CurrentVertex->neighbors[i] == destination)	
			{
				free(stack);
				return 1;
			}
			/* Otherwise check to see if the Vertex has not been visited */
			else if (CurrentVertex->neighbors[i]->isVisited == 0)
			{
				/* Add neighbor to the stack to be investigated for future path */
				addFrontCirListDeque(stack, CurrentVertex->neighbors[i]);
			}
		}
		/* Mark the Current Vertex as visited */
		CurrentVertex->isVisited = 1;

		/* Move to the next Vertex to visit */
		CurrentVertex = frontCirListDeque(stack);

		/* Consume (Pop) Vertex off the stack */
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	int i;
	/* Define the Temp Stack-Queue for the Function.
	* Going to use the Link List Deque full features */
	struct cirListDeque *stack = malloc(sizeof(struct cirListDeque));
	initCirListDeque(stack);
	...
	/* Add the source Vertex to the Stack for examination. */
	addBackCirListDeque(stack, CurrentVertex);
